 ...
 *
 * Centralizes rate limit state tracking, backoff calculation,
 * and error classification for both streaming and non-streaming handlers.
 */
...
export function getRateLimitBackoff(email, model, serverRetryAfterMs) {
    ...
    if (previous && now - previous.lastAt < RATE_LIMIT_DEDUP_WINDOW_MS) {
        ...
        logger.debug(
            `[CloudCode] Rate limit on ${email}:${model} within dedup window, attempt=${previous.consecutive429}, isDuplicate=true`
        );
        return {
    ...
    logger.debug(
        `[CloudCode] Rate limit backoff for ${email}:${model}: attempt=${attempt}, delayMs=${Math.max(baseDelay, backoffDelay)}`
    );
