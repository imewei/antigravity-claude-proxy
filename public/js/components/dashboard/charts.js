...
window.DashboardCharts.createDataset = function (label, data, color, canvas) {
    ...
    try {
        ...
        if (canvas && canvas.getContext) {
            ...
            if (ctx && ctx.createLinearGradient) {
                ...
                gradient.addColorStop(0.6, window.DashboardCharts.hexToRgba(color, 0.05));
    ...
    const backgroundColor = gradient || window.DashboardCharts.hexToRgba(color, 0.08);
...
window.DashboardCharts.updateCharts = function (component) {
    ...
    component.stats.overallHealth =
        totalModelCount > 0 ? Math.round(totalHealthSum / totalModelCount) : 0;
...
window.DashboardCharts.updateTrendChart = function (component) {
    ...
    console.log('[updateTrendChart] Canvas is ready, proceeding with chart creation');
